...
### [[系统调用io 文件io#1. 文件描述符的概念|一 文件描述符的概念]]

### [[系统调用io 文件io#2. 文件io操作相关函数：|二 文件IO操作]]
...
> 不可分割的操作
- 原子：不可分割的最小单位
- 作用：解决竞争和冲突    **应用**：多进程与多线程并发`tmpnam` 给文件名 操作不原子，名字 创建 两步 ；tmpfile 直接创建一个文件。创建[[临时文件]] 多种方法。
###  [[系统调用io 文件io#5. dup|七 程序中的重定向：dup,dup2]]
	- `dup.c`
### [[系统调用io 文件io#6. sync fsync|八 同步]]
> 与设备相关。 系统开发，中间层 sync,fsync,fdatasync
### [[系统调用io 文件io#7. fcnctl|九 fcntl()]]
> 管家级函数 管理文件描述符
