...
1. $\mathrm{GOTO}(I_i, a) = I_j \wedge a \in T \Rightarrow \mathrm{ACTION}[i, a] \leftarrow s_j$
2. $\mathrm{GOTO}(I_i, A) = I_j \wedge A \in N \Rightarrow \mathrm{GOTO}[i, A] \leftarrow g_j$
   终结符 shift;非终结符 goto
3. $[k: A \rightarrow \alpha \cdot] \in I_i \wedge A \neq S' \Rightarrow \forall t \in T \cup \{\char"24\}.\,\mathrm{ACTION}[i, t] = r_k$
4. $[S' \rightarrow S \cdot] \in I_i \Rightarrow \mathrm{ACTION}[i, \char"24] \leftarrow acc$
   归约，第四个特殊的归约，增广产生式。
